    ...
    >
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-link
            type="primary"
            :href="'/user/taskmachine/listbymachine?id=' + scope.row.id"
            target="_blank"
            >查看接单</el-link
          >
          
          <el-link type="primary" :href="'/user/taskcounter/listbymachine?id='+scope.row.id" target="_blank">查看日志</el-link>
          <el-link type="primary" :href="'/user/account/balancelogs?id='+scope.row.id" target="_blank">查看流水</el-link>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
export default {
  ...
  methods: {
    ...
    //获取当前页码的表格数据
    getList(data) {
      this.$api.user.machine.list(data).then(res => {
        if (res.code == 0) {
