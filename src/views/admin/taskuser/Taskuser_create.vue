...
export default {
  ...
  computed: {
    total: function() {
      return (this.ruleForm.price / 10000 * this.ruleForm.count) .toFixed(2);
    }
  },
  methods: {
    initData() {
      this.ruleForm.task_id = 10000;
      this.ruleForm.count = 1;
      this.ruleForm.price = 40000;
      this.ruleForm.url = "";
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          if(!this.ruleForm.video_id || !this.ruleForm.uid || !this.ruleForm.nickname || !this.ruleForm.url){
            ...
            this.$api.admin.taskuser.create(this.ruleForm).then(res=>{
              // console.log(res);
              if(res.code===0){
                this.initData();
                this.$message({
    ...
    async getMediaId() {
      ...
      var url = arr[0],
        data = await this.$api.admin.method.getmediaid({ url });
