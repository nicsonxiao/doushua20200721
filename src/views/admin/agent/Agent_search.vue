<template>
    ...
    >
      ...
      <el-form-item v-if="searchData.user_type===3">
        ...
        <el-button type="primary" @click="onSearchBalancelogs()">查看流水日志</el-button>
      </el-form-item>
    </el-form>
    </div>
...
export default {
    ...
    mounted(){
        if(this.$route.query.keyword){
            ...
            this.$api.admin.agent.search({keyword:this.$route.query.keyword}).then(res=>{
                // console.log(res);
                if(res.code==0){
    ...
    methods:{
        ...
        onSearchOrderDetails(){
            if(this.$route.query.keyword){
                ...
                path: "/admin/taskmachine/listbymachine",
                query: { id:this.searchData.id }
            });
        ...
        onSearchBalancelogs(){
            if(this.$route.query.keyword){
                let routeUrl = this.$router.resolve({
                path: "/admin/account/balancelogs",
                query: { id:this.searchData.id }
            });
            window.open(routeUrl.href, "_blank");
            }
