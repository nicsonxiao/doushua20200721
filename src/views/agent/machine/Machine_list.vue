  ...
  <div>
    ...
    <el-table :data="tableData" border style="width: 100%" stripe>
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-link type="primary"  :href="'/agent/taskmachine/listbymachine?id='+scope.row.id" target="_blank">查看接单</el-link>
          <el-link type="primary" :href="'/agent/taskcounter/listbymachine?id='+scope.row.id" target="_blank">查看日志</el-link>
          <el-link type="primary" :href="'/agent/account/balancelogs?id='+scope.row.id" target="_blank">查看流水</el-link>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
...
export default {
  ...
  mounted() {
    ...
    if(this.$route.query.id){
      this.$api.agent.agent.read({id:this.$route.query.id}).then(res=>{
        // console.log(res);
        if(res.code===0){
  ...
  methods: {
    ...
    //获取当前页码的表格数据
    getList(data) {
      this.$api.agent.machine.list(data).then(res => {
        if (res.code == 0) {
    ...
    onChangAgent() {
      ...
      this.$api.agent.agent.accounts(acountsObj).then(res => {
        if (res.code === 0) {
          this.userArr = res.data;
          if(this.$route.query.id){
            this.ruleForm.user_id = +this.$route.query.id;
