...
export default {
  ...
  methods: {
    ...
    async MachineTopup(id, card) {
      let _this = this,
        now = Date.now() / 1000,
        resMachineTopup = await _this.$api.agent.card.machineTopup({
          machine_id: id,
          card
        });
    ...
    async MachineTopupUser(user_login, card) {
      let now = Date.now() / 1000,
        res = await this.$api.agent.agent.search({
          keyword: user_login
        });
      if (res.code === 0) {
        let resMachineTopup = await this.$api.agent.card.machineTopup({
