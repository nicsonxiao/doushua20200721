<template>
  <div>
    <el-form
      ...
      class="demo-ruleForm"
    >
      <el-form-item>机器充值记录</el-form-item>
...
export default {
  ...
  methods: {
    ...
    //获取数据
    async getLogsbymachine(data) {
      let res = await this.$api.agent.card.logsbymachine(data);
      // console.log(res);
      if (res.code === 0) {
    ...
    onlookUp(formName) {
      ...
      _this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          _this.getLogsbymachine(dataObj);
        } else {
